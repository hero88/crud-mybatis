...
public class TimeTrackingServiceImpl implements TimeTrackingService {
    ...
    public TimeTrackingResponse saveTimeTracking(TimeTrackingRequest timeTrackingRequest) {
        ...
        try {
            ...
            Payroll payrollEmployee = payrollMapper.getPayrollByEmployeeId(timeTracking.getEmployeeId());
            TaxInformation taxInformation = taxInformationMapper.getTaxInformationByEmployeeId(timeTracking.getEmployeeId());
            ...
            if (dayOfWeekValue == 7) {
                ...
                if (clockOut < 6) {
                    ...
                    double firstPartSalary = getNetSalary(payrollEmployee, firstPartHours, 1.5, taxInformation.getTaxRate());
                    double secondPartSalary = getNetSalary(payrollEmployee, secondPartHours, 1.0, taxInformation.getTaxRate());
    ...
    public Double getNetSalary(Payroll payroll, Double hour, Double bonus, Double taxRate) {
        System.out.println(payroll.getSalary() * bonus * hour * (100 - taxRate));
        return payroll.getNetSalary() + payroll.getSalary() * bonus * hour * (100 - taxRate);
    }
